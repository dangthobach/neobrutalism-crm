...
public class CertificateService {
    ...
    public Certificate regeneratePDF(UUID certificateId) {
        ...
        try {
            ...
            log.info("PDF regenerated successfully for certificate: {}", certificate.getCertificateNumber());

        } catch (IOException e) {
            log.error("Failed to regenerate PDF for certificate {}: {}", certificateId, e.getMessage(), e);
            throw new BusinessException("Failed to regenerate certificate PDF: " + e.getMessage());
        }
    ...
    private Certificate createCertificate(Enrollment enrollment) {
        ...
        String verificationUrl = pdfGenerator.generateVerificationUrl(certificateNumber);
        ...
        try {
            ...
            log.error("Failed to generate PDF for certificate {}: {}", certificateNumber, e.getMessage(), e);
            // Set placeholder URL - can regenerate later
            certificate.setPdfUrl(String.format("%s/certificates/%s.pdf", applicationBaseUrl, certificateNumber));
        }
