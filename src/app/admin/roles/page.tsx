...
import { generateRoles } from "@/lib/mock"

export default function RolesPage() {
  const [rows, setRows] = useState<Role[]>([])
  useEffect(() => {
    setRows(generateRoles())
  }, [])
    ...
    <div className="space-y-4">
      ...
      <div className="border-4 border-black bg-background p-4 shadow-[8px_8px_0_#000]">
        <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
          <Input
            placeholder="Search roles by any field..."
            value={globalFilter ?? ""}
            onChange={(e) => setGlobalFilter(e.target.value)}
            className="max-w-md"
          />
          <div className="sm:ml-auto">
            <Button variant="noShadow" onClick={onCreate}>New Role</Button>
          </div>
        </div>

        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Input placeholder="Filter name..." value={(table.getColumn("name")?.getFilterValue() as string) ?? ""} onChange={(e) => table.getColumn("name")?.setFilterValue(e.target.value)} />
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-heading">{editing?.id ? "Edit Role" : "Create Role"}</DialogTitle>
          </DialogHeader>
          <div className="grid sm:grid-cols-2 gap-3">
            <Input placeholder="Name" value={editing?.name ?? ""} onChange={(e) => setEditing((u) => ({ ...(u as Role), name: e.target.value }))} />
            <Input placeholder="Description" value={editing?.description ?? ""} onChange={(e) => setEditing((u) => ({ ...(u as Role), description: e.target.value }))} />
          </div>
          <DialogFooter>
            <Button variant="noShadow" onClick={() => setOpen(false)}>Cancel</Button>
            <Button variant="noShadow" onClick={saveRole}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
